...
| `test_dynamic_constraint_diagnostics.py` | Tests for the dynamic constraint diagnostics example: the translating/rotating-line and expanding-circle constraint-preservation residuals (zero for compatible velocities, nonzero for incompatible ones), the affine compatible-velocity space (unique/underdetermined/inconsistent cases, rank-nullity sanity), and representation-rescaling invariance of rank and the particular solution. |
...
| `dynamic_constraint_diagnostics.md` | Standard DAE / constrained-dynamics structure extending `constraint_intersection_diagnostics.md` to moving or state-coupled constraints: the constraint-preservation equation, the affine compatible-velocity space, a representation-invariance table, and a four-level moving-constraint taxonomy (representation change / rigid motion / deformation / degeneration). Explicit M5 and GR firewalls; no dynamics solver. |
...
| `dynamic_constraint_diagnostics.py` | Executable example: exact local residuals for the translating-line, rotating-line, and expanding-circle constraint-preservation equations; the affine compatible-velocity space (particular solution + kernel) for a general `A xdot = b`, raising rather than silently approximating when incompatible; and a representation-rescaling invariance check. No integrator or simulation. Runs directly with `python examples/dynamic_constraint_diagnostics.py`. |
